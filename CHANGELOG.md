...
- packages/contracts publish to GitHub Packages is still a fully manual step (cd packages/contracts && npm publish). Nothing in CI or git hooks verifies that a package.json version bump was actually published, or that consumer pins (browchar-api's own dependency, browchar-fe's) track the published version. This already caused real drift: the 0.2.0 bump in this branch left browchar-api/package.json pinned to ^0.1.0, which doesn't even satisfy 0.2.0 under semver. Tracked as new requirements on DEV-123 (GitHub Actions ticket): a push-to-main publish-contracts.yml workflow, idempotent (skips if the version is already published), using secrets.GITHUB_TOKEN with packages:write, plus a check that FE/API pins match the newly published version.

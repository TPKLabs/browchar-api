...
- Validación de entradas: nunca confiar en `req.body`, `req.params` ni `req.query`. Este NestJS usa schemas compartidos de `@tpklabs/browchar-contracts`, DTOs con `createZodDto` y el `ZodValidationPipe` global; no introducir class-validator/class-transformer.
